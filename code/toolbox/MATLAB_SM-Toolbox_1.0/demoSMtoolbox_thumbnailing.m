...
clear; close all;
filename = 'Mozart_KV306-01_102_20080707-SMD.wav';%'Test_AABA.wav';
...
%--------------------------------------------------------------------------%
[fitness_info,parameter] = SSM_to_scapePlotFitness(S_final, parameter);     % 算 ALL fitness
...
[h_fig_scapeplot,x_axis,y_axis] = visualizeScapePlot(fitness_matrix,paramVisScp); % 印ALL fitness
...
[thumb_frame] = scapePlotFitness_to_thumbnail(fitness_matrix,parameter);    %  fitness 最大值的片段中心點及長度
...
hold on;
plot(point_x_pos,point_y_pos,'o','LineWidth',2,'color',[1, 0.5, 0]); % 畫圈圈
hold off;
...
[induced_frame,pathFamily] = thumbnailSSM_to_pathFamily(thumb_frame,S_final,parameter); % 找induced segment
...
groundTruth_struct = parseAnnotationFile([dirAnnotation parameter.title '.txt']); % 讀Ground truth

paramVisSegFam.segType = 'groundtruth';
visualizeSegFamily(groundTruth_struct,paramVisSegFam);  % plot Ground truth
title('Ground truth segmentation');
h_fig = gcf;
makePlotPlayable(f_audio, h_fig, parameterMPP);         % score following
